...
/// The bearer-token secret gating `MCPRemoteServer` (`Sentry/App/
/// MCPRemoteServer.swift`) — the in-app HTTP transport that lets an MCP
/// client reach Sentry's tools over the LAN, unlike the local-only stdio
/// `SentryMCP` binary. Stored in the login Keychain, never in
...
public enum MCPRemoteAccessKey {
    ...
    private static let service = "dev.malekswilam.sentry.mcp.remotekey"
