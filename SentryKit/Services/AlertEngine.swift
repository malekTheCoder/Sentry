...
public final class AlertEngine {
    ...
    private func currentCondition(for rule: AlertRule, snapshot: SystemSnapshot, now: Date) -> (Bool, Double?) {
        ...
        if let processName = rule.processNameMatch {
            // A `processRulesUnlocked` gate used to stand here, returning
            // an honest-nil "condition not met" for every process-scoped
            // rule on an unlicensed copy — the rule was kept but never
            // evaluated. Process-match rules are evaluated for everyone
            // now, so the only thing between a rule and a notification is
            // whether its condition is actually true.
