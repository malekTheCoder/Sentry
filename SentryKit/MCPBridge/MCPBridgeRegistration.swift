...
/// user as the same sentence: "Couldn't reach Sentry.app … Is Sentry
...
/// distinguishable in the UI **and** in `sentryctl`'s stderr is a requirement of
...
public enum MCPBridgeRegistration: Equatable, Sendable {
    ...
    /// state in which `sentryctl` and the stdio MCP server can work.
    ///
    ...
    /// just had `sentryctl` fail on them and does not know what a LaunchAgent
    /// is.
    public var explanation: String {
        switch self {
        case .registered:
            return "macOS will start Sentry's command-line bridge on demand, so the `sentryctl` tool and the stdio MCP server can reach this app. Sentry itself still has to be running for them to get an answer."
        case .awaitingApproval:
            return "macOS has accepted Sentry's command-line bridge but hasn't allowed it to run yet. Open System Settings ▸ General ▸ Login Items & Extensions and switch Sentry's background item on. Until then, `sentryctl` and the stdio MCP server can't reach this app."
        case .notRegistered:
            return "The `sentryctl` command-line tool and the stdio MCP server reach this app through a small background item that macOS starts on demand. It isn't registered yet, so both will fail even while Sentry is running. Setting it up registers one login item and starts no process until something actually connects.\n\nThis is separate from Remote Access above, which does not use it."
        case .unavailable(let reason):
            return "macOS refused to register Sentry's command-line bridge: \(reason)\n\nThe usual cause is a copy of Sentry that wasn't signed with a Developer ID certificate — macOS only registers a background item whose signature matches the app asking for it. Everything else in Sentry works normally; only the `sentryctl` tool and the stdio MCP server are affected. Remote Access, above, does not go through this path and is unaffected."
        }
    }
}
