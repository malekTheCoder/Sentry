...
/// `SentryXPCServiceProtocol`.** See `MCPBridgeContract`'s header for the
...
/// `SentryXPCServiceProtocol`'s own methods, directly from the tool to
/// Sentry over the connection this protocol hands out, never through here.
///
/// **`@objc` and the reply-block shape**, same constraint
/// `SentryXPCServiceProtocol` documents at length: `NSXPCConnection`
...
@objc public protocol MCPBridgeProtocol {
    ...
    /// `sentryctl` call would fail with a *less* accurate message than the one
    /// it deserves.
    func withdraw(reply: @escaping (Bool) -> Void)

    /// `sentryctl` / `SentryMCP` ask where Sentry is.
    ///
