...
/// mirroring `Sentry/Dashboard/TimeRangePicker.swift`'s enum-case spirit for
/// cross-app consistency without literally importing that type — it lives in
/// the `Sentry` *app* target, and Xcode app targets can't import each other
/// (same constraint `SentryMobile/Theme/ThemeColor+SwiftUI.swift`'s doc
/// comment documents for theming, `SentryMobile/Dashboard/MetricFormatting.swift`
/// for formatting).
///
/// **Why this pure enum lives in `SentryKit`, not next to the SwiftUI
/// segmented control that renders it in `SentryMobile/History/`.**
/// `SentryTests` (`project.yml`) depends on `SentryKit_macOS` and `Sentry`
/// only — there is no iOS test target in this build, so nothing under
/// `SentryMobile/` is reachable from a test. `Freshness.swift` in this same
...
/// "the `Picker` that renders it" (`SentryMobile/History/HistoryRangeSelector.swift`,
