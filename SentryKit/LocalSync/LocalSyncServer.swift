...
/// either. `LocalSyncClient` (`SentryKit/LocalSync/LocalSyncClient.swift`)
...
/// (`SentryKit/Services/LocalCommandExecutor.swift`).
///
...
/// it. Reachability from outside the LAN used to be gated per-peer at
/// accept time; it is not gated by anything any more, so an authenticated
/// peer is accepted wherever it connects from. The pairing code is the
/// whole authorization story, which is what the trust model above always
/// said it was.
...
public final class LocalSyncServer: @unchecked Sendable {
    ...
    private func accept(_ connection: NWConnection, isAuthenticated: Bool) {
        ...
        queue.async { [weak self] in
            ...
            // An entitlement check used to sit here, refusing any
            // authenticated peer whose source address wasn't on this Mac's
            // own network unless the copy was licensed for Remote Access.
            // Nothing replaces it: a peer that completed the TLS-PSK
            // handshake proved it knows the pairing code, and that has
            // always been the only fact this listener's trust model
            // actually relies on. The address-classification helpers went
            // with it — they existed solely to decide who to refuse.
