...
public struct AppSettings: Codable, Equatable, Sendable {
    ...
    /// Master switch for `MCPRemoteServer` (`Sentry/App/MCPRemoteServer.swift`)
    /// — an in-app HTTP transport reachable from other devices on the LAN,
    /// distinct from `mcpServerEnabled`'s local-only stdio `SentryMCP`
    /// binary. Off by default: this is the one MCP-related switch that
    /// actually changes Sentry's network posture (see `AIAccessPane`'s
