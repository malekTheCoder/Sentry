...
/// The client half of command-line access: how `sentryctl` and `SentryMCP`
/// reach a running Sentry.
///
/// **Read `SentryKit/MCPBridge/MCPBridgeContract.swift` first.** This type
...
/// callers (`sentryctl statusline`, notably) distinguish a connection-level
...
/// the proxy is not.
public final class SentryXPCClient: MCPServiceCalling, @unchecked Sendable {

    /// Every connection-level failure message starts with this.
    ///
    /// Exported rather than left as a string literal because `SentryCLI`
    ...
    public func readCall(_ body: @escaping (SentryXPCServiceProtocol, @escaping (Data?, String?) -> Void) -> Void) async -> (Data?, String?) {
        ...
        case .connected(let connection):
            return await withCheckedContinuation { (continuation: CheckedContinuation<(Data?, String?), Never>) in
                ...
                }) as? SentryXPCServiceProtocol else {
    ...
    public func writeCall(_ body: @escaping (SentryXPCServiceProtocol, @escaping (Bool, String?) -> Void) -> Void) async -> (Bool, String?) {
        switch await broker.connection() {
        case .unreachable(let reason):
            return (false, reason)
        case .connected(let connection):
    ...
    static let mismatchedInterface =
        "\(unreachablePrefix): it answered with something this tool doesn't recognise as Sentry's interface. This `sentryctl` and Sentry.app are probably different versions."
}
...
/// the reason `SentryXPCServiceProtocol` documents at length (an `@objc` XPC
...
private actor ConnectionBroker {
    ...
    func connection() async -> MCPRendezvous {
        ...
        case .resolved(let endpoint):
            ...
            connection.remoteObjectInterface = NSXPCInterface(with: SentryXPCServiceProtocol.self)
            connection.resume()
    ...
    /// long-lived `SentryMCP` session lasts. Sentry holds the connection that
    ...
    private func resolveEndpoint() async -> MCPEndpointLookup {
        ...
        return await withCheckedContinuation { (continuation: CheckedContinuation<MCPEndpointLookup, Never>) in
            ...
            // `sentryctl` is run from shell prompts and `PreToolUse` hooks, where
            ...
            proxy.resolveAppEndpoint { endpoint, message in
                guard let endpoint else {
                    ...
                    box.resume(.unreachable(
                        "\(SentryXPCClient.unreachablePrefix): \(message ?? "its command-line bridge is running but had no connection point for Sentry.")"
                    ))
                    return
                }
    ...
    /// The message that replaces "Is Sentry running?" — the wrong question,
    ...
    static func bridgeUnreachable(_ error: Error) -> String {
        """
        \(SentryXPCClient.unreachablePrefix): its command-line bridge isn't available (\(error.localizedDescription)).

        `sentryctl` and the stdio MCP server don't talk to Sentry directly — they go through a small background item that macOS starts on demand, and it has to be set up once. Sentry running is not enough on its own.
    ...
    static let bridgeTimedOut =
        "\(SentryXPCClient.unreachablePrefix): its command-line bridge didn't answer within \(Int(MCPBridgeTiming.resolveTimeout)) seconds. It may be wedged; quitting and reopening Sentry, or logging out and back in, will restart it."
}
