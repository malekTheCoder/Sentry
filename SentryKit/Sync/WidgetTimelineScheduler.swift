...
/// `SentryWidget`'s `TimelineProvider` so it's testable without a
/// WidgetKit-hosting process — `SentryWidgetExtension` isn't a dependency
/// of `SentryTests` (nor should it become one just for this), and
...
/// core; `Provider` in `SentryWidget/Provider.swift` is the thin WidgetKit
/// adapter around it, matching this codebase's existing split between pure
/// logic in `SentryKit` and thin platform glue in the app targets (e.g.
