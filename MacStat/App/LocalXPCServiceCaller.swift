...
/// Calls `MacStatXPCServiceProtocol` methods directly against an in-process
/// object — no `NSXPCConnection`, no Mach service lookup. See
/// `MCPServiceCalling`'s doc comment for why: `MCPRemoteServer` lives inside
/// `MacStat.app` already, and self-connecting via `NSXPCConnection` to a
...
/// every `MacStatXPCServiceProtocol` method is `nonisolated` on the
...
final class LocalXPCServiceCaller: MCPServiceCalling, @unchecked Sendable {
    ...
    init(service: MacStatXPCServiceProtocol) {
