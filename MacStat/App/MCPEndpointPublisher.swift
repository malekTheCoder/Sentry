...
///
/// **Read `MacStatKit/MCPBridge/MCPBridgeContract.swift` first.** In one
/// paragraph: `AppDelegate` used to publish a Mach service name that nothing
/// had registered with launchd, so `macstat` and the stdio MCP server could
...
final class MCPEndpointPublisher: ObservableObject {
    ...
    private static let logger = Logger(
        subsystem: "dev.malekswilam.macstat",
    ...
    init(
        service: MacStatXPCServiceProtocol,
    ...
    /// user chooses Quit — retire the entry immediately, so a `macstat` run a
...
/// stall an incoming connection — and the whole point of `macstat statusline`
...
private final class MCPAppEndpointVendor: NSObject, NSXPCListenerDelegate {
    ...
    private let service: MacStatXPCServiceProtocol
    private let evaluator: MCPBridgePeerEvaluator
    private let ownPID = getpid()

    init(service: MacStatXPCServiceProtocol, evaluator: MCPBridgePeerEvaluator) {
    ...
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        ...
        // inside it, against live settings — see `MacStatXPCServiceProtocol`'s
        // doc comment. This gate answers "may you connect", never "may you do
        // this".
        connection.exportedInterface = NSXPCInterface(with: MacStatXPCServiceProtocol.self)
