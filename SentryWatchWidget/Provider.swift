...
// MARK: - SentryWatchWidgetEntry

/// One point on the complication's timeline — mirrors
/// `SentryWidget/Provider.swift`'s `SentryWidgetEntry` shape and reasoning
...
/// (`SentryKit/Watch/WatchRelayStore.swift`). Deliberately almost no logic
/// of its own, matching `SentryWidget/Provider.swift`'s precedent.
///
...
struct Provider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> SentryWatchWidgetEntry {
        SentryWatchWidgetEntry(date: Date(), snapshot: Self.placeholderSnapshot)
    }

    func getSnapshot(in context: Context, completion: @escaping (SentryWatchWidgetEntry) -> Void) {
        let snapshot = WatchRelayStore.read() ?? (context.isPreview ? Self.placeholderSnapshot : nil)
        completion(SentryWatchWidgetEntry(date: Date(), snapshot: snapshot))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<SentryWatchWidgetEntry>) -> Void) {
        ...
        let entries = dates.map { SentryWatchWidgetEntry(date: $0, snapshot: snapshot) }
