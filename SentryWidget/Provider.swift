...
struct SentryWidgetEntry: TimelineEntry {
    let date: Date
...
/// cadence, `SentryKit/Sync/WidgetTimelineScheduler.swift`) and
/// `WidgetSnapshotStore` (App Group read, `SentryKit/Sync/WidgetSnapshot.swift`).
...
/// unit-tested `SentryKit` type; this just wires WidgetKit's callback shape
/// to those.
struct Provider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> SentryWidgetEntry {
        SentryWidgetEntry(date: Date(), snapshot: Self.placeholderSnapshot)
    }
    ...
    /// than honestly stale.
    func getSnapshot(in context: Context, completion: @escaping (SentryWidgetEntry) -> Void) {
        let snapshot = WidgetSnapshotStore.read() ?? Self.placeholderSnapshot
        completion(SentryWidgetEntry(date: Date(), snapshot: snapshot))
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<SentryWidgetEntry>) -> Void) {
        let snapshot = WidgetSnapshotStore.read()
        let now = Date()
        let dates = WidgetTimelineScheduler.reloadDates(from: now)
        let entries = dates.map { SentryWidgetEntry(date: $0, snapshot: snapshot) }
