...
//
// **Read this before anything else in `MacStatKit/MCPBridge/`.**
//
...
//   2. `macstat` / `MacStatMCP` connect to it, ask for that endpoint, and
...
// re-implement all twenty-odd methods of `MacStatXPCServiceProtocol`, would
...
// nothing prompts for an administrator password at any point, and `macstat`
// fails with a message that names the *real* cause instead of the old
// "Is MacStat running?" misdirection.
//
// ## Relationship to `MacStatKit/FanDaemon/`
//
...
//   * **The concrete `SecCode` evaluator lives in `MacStatKit`, not in the
...
public enum MCPBridgeNaming {
    ...
    ///
    /// **This is the same string as `MacStatXPCServiceName.machService`, on
    /// purpose.** That constant is the client-facing name and predates this
    /// file; keeping it means no `macstat` invocation, MCP config, or
...
public enum MCPBridgeTiming {
    ...
    /// *next* `macstat` invocation pay a cold-launch penalty for no benefit.
    ...
    /// later one is a table lookup. A `macstat statusline` call running in
    /// somebody's shell prompt has its own much tighter budget (50 ms, see
    /// `MacStatCLI`) and gives up long before this fires — this timeout
