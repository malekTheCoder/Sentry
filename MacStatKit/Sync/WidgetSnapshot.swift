...
/// hardcoded label in `MacStatWidget/`.
///
/// **Why this lives in `MacStatKit`, not `MacStatMobile` or
/// `MacStatWidget`.** Both the phone app (writer) and the widget extension
...
/// drift). `MacStatKit_iOS` is already a dependency of both
...
/// factored out of `MacStatMobile`'s `WidgetSnapshotWriter` so it's testable
/// from `MacStatTests` without constructing an actor, a `MockDataSource`, or
...
/// `MacStatMobile`, `MacStatWidget`'s `TimelineProvider`) need to treat as
...
public enum WidgetSnapshotStore {
    ...
    /// Must match `MacStatMobile.entitlements` and `MacStatWidget.entitlements`
