...
/// (Mac: `MacStat/Dropdown/ThemeColor+SwiftUI.swift`; iOS: the equivalent
/// file in `MacStatMobile/`) resolves a `Theme` against a `ColorScheme` and
/// lives in the *app* targets, not `MacStatKit` — it isn't available to a
/// framework-level view without inverting that dependency (app targets
/// depend on `MacStatKit`, not the reverse). Freshness's own color meanings
...
///
/// **Why this lives in `MacStatKit` despite being the framework's first
...
/// dependency — it compiles for both the `MacStatKit_macOS` and
/// `MacStatKit_iOS` framework targets exactly like the rest of this file's
