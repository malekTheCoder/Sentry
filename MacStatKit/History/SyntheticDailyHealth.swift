...
/// for `MacStatMobile/History` to chart, in lieu of a real CloudKit query.
///
...
/// produce one: `CKMapper` (`MacStatKit/Sync/CKMapper.swift`) can convert a
...
///
/// **Why the math lives here, in `MacStatKit`, rather than inside
/// `MockDataSource` (`MacStatMobile/Data/MockDataSource.swift`), which is
/// the only thing that calls it.** `MockDataSource` is deliberately scoped
/// to `MacStatMobile` — it is an iOS-app-only fake transport. But
/// `MacStatTests` depends on `MacStatKit_macOS`/`MacStat` only (`project.yml`)
/// — there is no iOS test target in this build, so nothing defined inside
/// `MacStatMobile` is reachable from a test at all. Splitting "the
