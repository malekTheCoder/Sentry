...
/// happily leave an old `MacStatWatch` on the wrist for days after the
...
///   property for, so an older `MacStatWatch` decodes the v1 subset and
...
public struct WatchRelaySnapshot: Codable, Sendable, Equatable {
    ...
    /// two reasons. It keeps `MacStatKit_watchOS`'s source list from having
...
/// `MemoryPressureLevel` (`MacStatKit/Models/MemoryStats.swift`) — same
