...
/// The Watch-local cache `MacStatWatch`'s `WCSessionDelegate` writes into
/// after every relay received from the phone, and
/// `MacStatWatchWidgetExtension`'s complication `TimelineProvider` reads
...
/// solves on iOS between `MacStatMobile` and `MacStatWidgetExtension`, and
/// the same fix applies.
public enum WatchRelayStore {
    /// Shared between `MacStatWatch` and `MacStatWatchWidgetExtension`
    ...
    /// Called by `MacStatWatch`'s `WatchSessionController` on every
    ...
    /// by `MacStatWatch`'s `ContentView` to render its own minimal screen.
