...
/// extracted from `WatchRelayManager` (`MacStatMobile/Watch/WatchRelayManager.swift`).
///
...
/// `StatsTransport` — see `WatchRelayManagerTests` in `MacStatTests`". No
/// such test file has ever existed, and it could not have: `MacStatTests` is
/// a macOS-hosted bundle that links `MacStat` and `MacStatKit_macOS`, and
...
/// and then left somewhere no test could reach. Moving it into `MacStatKit`
/// — which `MacStatTests` already links, and which the manager already
...
/// `MacStatKit/Watch/` next to the payload it reasons about, which means the
/// watch-side framework variant (`MacStatKit_watchOS`, whose source list is
