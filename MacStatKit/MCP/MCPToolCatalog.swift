...
public enum MCPToolCatalog {
    ...
    private static func inputSchema(for id: MCPToolID) -> Value {
        ...
        case .keepAwake:
            return schema(
                properties: [
                    ...
                    "reason": property("string", "Human-readable reason shown in MacStat's UI.")
                ],
                required: ["mode"]
            )
        ...
        case .setAlertRuleEnabled:
            return schema(
                properties: [
                    "ruleID": property("string", "UUID of the alert rule (see get_alert_history / MacStat's Alerts pane)."),
        ...
        case .createAlertRule:
            return schema(
                properties: [
                    "ruleJSON": property(
                        "string",
                        "JSON object matching MacStat's AlertRule shape (minus id/isEnabled): name, metric, comparison (\"above\"|\"below\"|\"equals\"|\"changedBy\"), threshold, sustainedFor, cooldown, onlyWhen ([\"onBattery\"|\"charging\"|\"pluggedIn\"|\"displayAsleep\"]), actions (e.g. [{\"notification\":{\"title\":\"...\",\"body\":\"...\",\"sound\":false}}])."
                    )
                ],
                required: ["ruleJSON"]
            )
    ...
    private static func read(
        ...
        guard let data else {
            return errorResult(message ?? "MacStat denied this request.")
        }
