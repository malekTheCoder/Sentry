...
/// The one shared definition of the `macstat://pair` link that carries a
...
/// in MacStatKit next to `SyncSecurity` instead of being string-built in
...
public enum RemotePairing {
    ...
    /// URL scheme + host, e.g. `macstat://pair?host=…&port=…&code=…`.
    /// The iPhone target's Info.plist registers `macstat` under
    /// `CFBundleURLTypes`; changing either constant is a breaking change to
    /// already-printed/screenshotted QR codes.
    public static let scheme = "macstat"
    ...
    /// anything that isn't a well-formed `macstat://pair` URL with a
