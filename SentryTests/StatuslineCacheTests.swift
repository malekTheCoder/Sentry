...
/// Covers `StatuslineCache`, the one-file fallback `sentryctl statusline`
/// falls back to when `Sentry.app` can't answer inside its 50ms budget
...
final class StatuslineCacheTests: XCTestCase {
    ...
    func testStoreCreatesItsDirectory() {
        // `~/Library/Caches/dev.malekswilam.sentry/` does not exist on a
    ...
    func testTheDefaultLocationIsUnderCachesAndNotApplicationSupport() {
        ...
        XCTAssertTrue(path.hasSuffix("dev.malekswilam.sentry/statusline.json"), path)
    }
}
