...
/// iOS counterpart of `Sentry/Dropdown/ThemeColor+SwiftUI.swift` (the Mac
...
/// of the `Sentry` app target, not `SentryKit`, and Xcode/SwiftPM app
/// targets cannot import each other — only frameworks are shareable that
/// way. Moving it into `SentryKit` instead of duplicating it was
...
/// History/Alerts/Settings in parallel against today's `Sentry` target
...
extension ThemeColor {
    ...
    /// doc comment for why the *parsing* now lives in `SentryKit` while the
