...
public actor MockDataSource: StatsTransport {
    ...
    /// that type's doc comment for why (short version: `SentryTests` can't
    /// see anything defined inside `SentryMobile`, so the testable part has
    /// to live in `SentryKit`). This method is just the actor-isolated
