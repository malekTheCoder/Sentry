...
struct AboutView: View {
    ...
    private var privacyCard: some View {
        VStack(alignment: .leading, spacing: palette.spacingTight) {
            ...
            // This sentence used to end "the link above isn't live," which was
            // true when it was written and became false the moment the policy
            // was published. Shipping copy that tells a reviewer a required
            // link is dead is a self-inflicted rejection — App Review checks
            // the privacy policy URL on every submission — so the claim is
            // gone rather than reworded, and what remains is the part that is
            // durably true and actually useful to someone reading it in the
            // app: where their data lives.
            Text("Sentry keeps its data on your own devices. The full written policy is linked above.")
                .scaledFont(palette, size: 10.5)
                .foregroundStyle(palette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
        }
