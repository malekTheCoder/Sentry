...
/// iOS counterpart of `MacStat/Dropdown/MetricFormatting.swift` (the Mac app
/// target's nil-handling wrapper around `MacStatKit.MetricFormatter`).
///
/// **Why this is a separate file instead of sharing the Mac one.** Same
/// reasoning as `ThemeColor+SwiftUI.swift` in `MacStatMobile/Theme/`: the Mac
/// file's code is pure Foundation with no AppKit dependency and could run
/// unmodified here, but it's a member of the `MacStat` app target, not
/// `MacStatKit`, and Xcode app targets can't import each other. Duplicating
/// this one small, stable file (rather than promoting it into `MacStatKit`,
