...
/// In-app HTTP transport for Sentry's MCP tools — the LAN-reachable
/// counterpart to the local-only stdio `SentryMCP` binary. Off by default
...
/// intended client. `APIKeyValidator` (`SentryKit/MCP/APIKeyValidator.swift`)
...
public final class MCPRemoteServer: @unchecked Sendable {
    private static let logger = Logger(subsystem: "dev.malekswilam.sentry", category: "MCPRemoteServer")
    ...
    public func start(port: Int, serviceCaller: any MCPServiceCalling) async {
        ...
        let mcpServer = await SentryMCPServerFactory.makeServer(
...
/// shared across every connection, matching how a single `SentryMCP`
