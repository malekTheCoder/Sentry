...
// SentryMCP — the stdio JSON-RPC binary from plan §13.2. An MCP client
...
// already-running `Sentry.app` over a local `NSXPCConnection` to the Mach
// service `SentryXPCServiceName.machService`. It never touches IOKit,
// SQLite, or any of `StatsCoordinator`/`PowerControlService`/`HistoryStore`/
// `AlertEngine` directly — see `SentryXPCServiceProtocol`'s doc comment for
...
// (the local stdio/XPC-agnostic protocol library), and `SentryKit`. No
...
// remote transport `Sentry/App/MCPRemoteServer.swift` provides instead).
//
// The actual tool/resource dispatch wiring lives in `SentryKit/MCP/
// SentryMCPServerFactory.swift`, shared with `MCPRemoteServer` — this file
// is just the stdio-specific entry point now.

let xpcClient = SentryXPCClient()
let clientIdentity = MCPClientIdentity()
let subscriptionPump = ResourceSubscriptionPump(xpcClient: xpcClient, clientName: "Sentry (resource subscription)")

let server = await SentryMCPServerFactory.makeServer(
    xpcClient: xpcClient,
...
do {
    ...
    FileHandle.standardError.write("SentryMCP failed to start: \(error)\n".data(using: .utf8) ?? Data())
    exit(1)
}
