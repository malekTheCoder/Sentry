...
`SentryWidget/` (plus `SentryWatchWidget/`, which the widget audit made
obviously incomplete without). `SentryKit/` was touched only where reading
...
- `Intents/SentryIntents.swift` — every Siri dialog string. The battery
...
### SentryWatch (watch app) — previously unlocalizable

- **New catalog: `SentryWatch/Resources/Localizable.xcstrings`** (80
...
- `Intents/SentryWatchIntents.swift` — every Siri dialog string, battery
  sentence rebuilt as whole sentences per charging state.

### SentryWidget / SentryWatchWidget — previously unlocalizable

- **New catalogs: `SentryWidget/Resources/Localizable.xcstrings`** (18
  keys, shared by the iOS and macOS widget extension targets) **and
  `SentryWatchWidget/Resources/Localizable.xcstrings`** (6 keys). A
...
- `SentryWidgetBundle.swift` — gallery description (a `String` property,
...
- **`Sentry/Dropdown/SleepControlCard.swift:705` `assertionReason`** —
...
- **`Sentry/Dropdown/SleepControlCard.swift:735,751`
...
- **`SentryKit/Services/AlertEngine.swift:675–834` default rule names and
  ...
  belongs to a SentryKit pass with a look at what happens to
  already-persisted English rule names.
- **`Sentry/App/MCPXPCService.swift:275–276, 394–398`** — MCP payload
...
- **`Sentry/Debug/*`, `Sentry/Debug/DebugWindowController.swift:58`** —
  debug surfaces, excluded by charter.
- **`Sentry/MenuBar/StatusItemView.swift:393`** — `"Sentry: " +
  parts.joined(", ")` — brand name plus a data list; the parts are
  formatted metric values, not prose.
- **`SentryMobile/History/HistoryRangeSelector.swift:45–49`** —
  ...
  not coverage.
- **`SentryWatch/ContentView.swift:348–372` `WatchFormatting`** —
  `"38%"`, `"3h 40m"` numeric/unit output; `Duration.formatted()`
  territory.
- **`SentryMobile/Intents/SentryIntents.swift:252` /
  `SentryWatch/Intents/SentryWatchIntents.swift:269`** — the battery
  ...
  facts.
- **`Sentry/Dropdown/SystemVitals.swift` status reasons (`"\(title):
...
- **Interpolated `LocalizedStringKey` literals in the new-catalog targets**
  (e.g. `SentryWidget/Views/SmallWidgetView.swift:30` `Text("\(…)W")`) —
...
- **Preview/demo fixture strings** — `SentryWatch/WatchSessionController
  .swift:78,99`, `SentryWidget/Provider.swift:86`,
  `SentryWatchWidget/Provider.swift:64` ("Malek's MacBook Pro", "Mac
...
|---|---|---|
| `Sentry/Resources/Localizable.xcstrings` | 183 | 547 |
| `SentryKit/Resources/Localizable.xcstrings` | 120 | 120 (untouched) |
| `SentryMobile/Resources/Localizable.xcstrings` | 48 | 123 |
| `SentryWatch/Resources/Localizable.xcstrings` | — (didn't exist) | 80 |
| `SentryWidget/Resources/Localizable.xcstrings` | — (didn't exist) | 18 |
| `SentryWatchWidget/Resources/Localizable.xcstrings` | — (didn't exist) | 6 |
| **Total** | **351** | **894** |
