...
| Write the concrete `LicenseActivationClient` conformer (`SentryKit/Pro/LicenseActivation.swift` is the seam; two calls, `activate` and `revalidate`) | Human 3 (vendor's API shapes) |
...
| Flip the composition root: pass the real client and `revalidationPolicy: .standard` **together** at `Sentry/App/AppDelegate.swift:276–279` (flipping the policy alone locks every licensed user out after 14 days — see `LicenseRevalidationPolicy`) | the client above |
| Walk the Pro dry run, steps 1–11 (`pro-license-dryrun-checklist.md`) | Human 3, Human 4, the licence pane (below), D1 |
...
- **Pro licence pane in Settings.** `LicenseProEntitlementStore.installLicense`, `removeLicense`, and `decision` exist and are tested, but **no UI calls them** (grep `installLicense(` under `Sentry/` — zero hits). Needed: paste field, `LicenseDenialReason.explanation` display, remove button, seat count and issue date from the payload. Prerequisite 4 of the dry run; buildable and testable today with a throwaway test key, exactly as `SentryTests/LicenseProEntitlementTests.swift` does. Must ship in the same release as checkout.
