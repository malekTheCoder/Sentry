...
- [x] **Support URL — decided and compiled in.** ~~There is currently no support URL anywhere in the project.~~ `https://malekthecoder.github.io/Sentry/support` (the dedicated support page on `gh-pages`, 200, listing both Issues and `getsentryapp@gmail.com` plus how to find a crash log) is now `AppCredits.supportURLString` and linked from the iPhone About screen, so the ASC field and the in-app link share one definition. Entering it is part of the data-entry item above.
