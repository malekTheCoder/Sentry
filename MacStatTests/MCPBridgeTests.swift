...
final class MCPBridgePeerGateTests: XCTestCase {
    ...
    /// `macstat`'s stderr and the Settings pane. The unsigned-build case in
...
final class MCPBridgeRegistrationTests: XCTestCase {
    ...
    func testUnavailableScopesTheDamageHonestly() {
        ...
        XCTAssertTrue(explanation.contains("macstat"))
    }
}

final class MacStatXPCClientMessagingTests: XCTestCase {

    /// `MacStatCLI`'s `statusline` decides whether it may fall back to a
    ...
    func testEveryConnectionLevelMessageCarriesThePrefix() {
        ...
        let messages = [
            MacStatXPCClient.callFailure(error),
            MacStatXPCClient.mismatchedInterface
        ]
        for message in messages {
            XCTAssertTrue(
                message.hasPrefix(MacStatXPCClient.unreachablePrefix),
    ...
    /// MacStat running?" was wrong in the common case and sent people to check
    ...
    func testMessagesDoNotAskWhetherTheAppIsRunningAsTheFirstExplanation() {
        ...
        XCTAssertFalse(MacStatXPCClient.callFailure(error).contains("Is MacStat running?"))
        XCTAssertFalse(MacStatXPCClient.mismatchedInterface.contains("Is MacStat running?"))
    }
}
...
final class MCPEndpointPublisherTests: XCTestCase {
    ...
    /// A stub `MacStatXPCServiceProtocol` — the publisher only ever hands this
    /// to `NSXPCListener` as an exported object, so nothing here is called.
    private final class NullService: NSObject, MacStatXPCServiceProtocol {
