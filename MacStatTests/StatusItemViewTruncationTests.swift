...
/// `StatusItemView.rebuildLayout()` (MacStat/MenuBar/StatusItemView.swift):
...
final class StatusItemViewTruncationTests: XCTestCase {
    ...
    func testNilMaxWidthKeepsAllModulesAndAccessibilitySummary() {
        let view = StatusItemView(layout: manyModulesLayout(maxWidth: nil), theme: .system)
    ...
    func testVeryNarrowBudgetStillKeepsFirstModule() {
        ...
        let view = StatusItemView(layout: manyModulesLayout(maxWidth: 1), theme: .system)
        view.update(makeSnapshot())
    ...
    func testNarrowBudgetDropsTrailingModulesComparedToUnlimited() {
        let unlimited = StatusItemView(layout: manyModulesLayout(maxWidth: nil), theme: .system)
        unlimited.update(makeSnapshot())

        let narrow = StatusItemView(layout: manyModulesLayout(maxWidth: 60), theme: .system)
    ...
    func testWidthNeverDecreasesWhenBudgetGrows() {
        ...
        let view = StatusItemView(layout: layoutNarrow, theme: .system)
        view.update(makeSnapshot())
    ...
    func testEmptyModulesFallsBackToNonZeroWidth() {
        let view = StatusItemView(layout: MenuBarLayout(modules: [], maxWidth: nil), theme: .system)
        view.update(makeSnapshot())
    ...
    func testSingleModuleLayoutIsNeverEmptyEvenWithTinyBudget() {
        ...
        let view = StatusItemView(layout: layout, theme: .system)
        view.update(makeSnapshot())
