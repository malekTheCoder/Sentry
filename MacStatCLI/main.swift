...
func printUsage() {
    ...
    USAGE:
        ...
        macstat watch --metric <id> [--interval 1s]
            ...
            --unbuffered`, or a log file:
                macstat watch --metric cpu.total_percent --interval 2s | jq .value
            `--list-metrics` prints every streamable ID and its unit.
            MacStat's MCP rate limit (Settings → AI Access, 20/min by
            default) applies; `watch` widens its own interval when it is
            hit and reports the new cadence on stderr and in each line's
            `intervalSeconds`.

        macstat statusline [--format compact|plain|tmux|nerdfont]
            One short line for a tmux status bar, a Starship prompt, or
            Claude Code's status line. Gives up on MacStat after 50ms and
...
/// `macstat watch --metric cpu.total_percent`, `--format compact`).
///
...
/// `--name`" either way, which is the accurate message for `macstat watch
...
func optionValue(_ name: String, in args: [String]) -> String? {
    ...
    // `macstat statusline --format --json` should complain about a missing
...
//
// **SIGPIPE.** `macstat watch | head -3` closes the read end after three
...
enum Stdout {
    ...
    /// `macstat status | head -1` does today.
        ...
        while offset < bytes.count {
            ...
            default:
                FileHandle.standardError.write(
                    ("macstat: couldn't write to stdout: \(String(cString: strerror(errno)))\n").data(using: .utf8) ?? Data()
                )
                exit(1)
            }
        }
...
// MARK: - macstat watch (plan §21.2.1, final row)
...
/// contract carries denials as `(nil, String?)` — `MacStatXPCServiceProtocol`
...
func runWatch(metric: MetricID, requestedInterval: Double) async -> Never {
    ...
    while true {
        ...
        guard let data else {
            let reason = message ?? "MacStat didn't answer. Is MacStat running?"
            ...
            if widened > effectiveInterval {
                ...
                warn("rate-limited by MacStat; widening --interval to \(formatInterval(effectiveInterval)). Raise the limit in Settings → AI Access to stream faster.")
            }
        ...
        guard let snapshot = try? snapshotDecoder.decode(SystemSnapshot.self, from: data) else {
            fail("couldn't decode MacStat's snapshot — this `macstat` and MacStat.app are probably different versions.")
        }
...
// MARK: - macstat statusline (plan §21.3)
...
/// other `macstat` command is invoked deliberately and may take as long as
/// the machine takes; `statusline` is invoked before every prompt, by a
/// program that is not prepared to wait, and a hung `MacStat.app` must
/// degrade this command rather than the user's shell.
///
/// The budget covers the XPC round trip only. It is not, and cannot be, a
/// budget for the whole process: dyld resolving `MacStatKit.framework` and
...
/// config distinguish "MacStat is briefly busy" from "MacStat is gone."
...
func runStatusline(format: StatuslineFormat, budgetMillis: Double) async -> Never {
    ...
    case .timedOut:
        renderFromCacheOrExit(
            ...
            format: format,
            theme: theme,
            reason: "MacStat didn't answer within \(Int(budgetMillis))ms.",
    ...
    case .answered(.some(let data), _):
        ...
        guard !line.isEmpty else {
            // Reached MacStat, got a snapshot, and it contained no battery,
            ...
            warn("MacStat has no readable metrics yet — it may have just launched.")
            exit(1)
        }
        Stdout.writeLine(line)
        exit(0)
...
case "watch":
    if arguments.contains("--list-metrics") {
        ...
        Stdout.ignoreSIGPIPE()
        // stdout, because this is data: `macstat watch --list-metrics |
    ...
    guard let rawMetric = optionValue("metric", in: arguments) else {
        fail("missing --metric <id>. Run `macstat watch --list-metrics` to see them all.")
    }
    guard let metric = MetricID(rawValue: rawMetric) else {
        ...
        if rawMetric.hasPrefix("cpu.core.") || rawMetric.hasPrefix("thermal.fan_") {
            fail("`\(rawMetric)` is a per-core/per-fan metric, which `watch` can't stream — only the enumerable metrics in `macstat watch --list-metrics` can be read out of a snapshot.")
        }
        fail("unknown metric `\(rawMetric)`. Run `macstat watch --list-metrics` to see them all.")
    }
